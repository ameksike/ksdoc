...
class SchemaController extends ksdp.integration.Dip {
    ...
    async save(req, res) {
        ...
        let { content, schema, extension } = params;
        if (!content) {
        ...
        try {
            const filename = await this.contentService?.save({ content, schema, extension });
            this.logger?.info({
                flow: req.flow,
                src: 'KsDoc:Schema:save',
                data: { filename }
            });
            res.send({ success: true, msg: 'SAVE_OK', data: { schema, extension } });
        }
        catch (error) {
            this.logger?.error({
                flow: req.flow,
    ...
    async delete(req, res) {
        try {
            ...
            let filename = await this.contentService?.delete({ filename: id, extension: '.html', schema });
            this.logger?.info({
                flow: req.flow,
